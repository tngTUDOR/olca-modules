...
public class LinkContributionsTest {
	...
	public void testDoubleLink() {
		...
		graph.putLink(LongPair.of(2, 4), LongPair.of(4, 4));
		graph.putLink(LongPair.of(3, 4), LongPair.of(4, 4));
	...
	public void testBandMatrix() {
		...
		for (int i = 0; i < size; i++) {
			graph.index.put(LongPair.of(i + 1, i + 1));
			techMatrix.setEntry(i, i, 1.0);
			s[i] = 1;
			if (i < (size - 1)) {
				techMatrix.setEntry(i + 1, i, -1);
				graph.putLink(LongPair.of(i + 1, i + 2),
						LongPair.of(i + 2, i + 2));
			}
		}
